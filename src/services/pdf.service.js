...
async function generateTransactionPDF(transactions, walletId) {
  ...
  try {
    ...
    transactions.forEach((transaction, index) => {
      if (index % 2 === 0) {
        doc.rect(startX, rowY, pageWidth, rowHeight)
          .fill('#f7f7f7');
      }
      ...
      doc.text(formatDate(transaction.date), 
        currentX + 5, rowY + 6, {
      ...
      doc.text(formatCurrency(transaction.amount),
        currentX + 5, rowY + 6, {
      ...
      doc.text(formatCurrency(transaction.balance),
        currentX + 5, rowY + 6, {
    ...
    doc.moveDown(2)
      .fontSize(12)
      .text('End of Statement', { align: 'center' });
